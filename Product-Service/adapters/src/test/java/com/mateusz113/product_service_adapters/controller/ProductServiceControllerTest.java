...
public class ProductServiceControllerTest {
    ...
    void getProducts_ReturnsPageableContentDtoWithStatus200() throws Exception {
        ...
        when(getProducts.getAll(pageable.getPageNumber(), pageable.getPageSize())).thenReturn(productPageableContent);
    ...
    @Test
    void getProductsByType_ReturnsPageableContentDtoWithStatus200() throws Exception {
        Pageable pageable = PageRequest.of(0, 3);
        String type = "type";
        PageableContent<Product> productPageableContent = getProductPageableContent(pageable);
        when(getProducts.getByType(type, pageable.getPageNumber(), pageable.getPageSize())).thenReturn(productPageableContent);
        mockMvc.perform(get("/products/type/{type}", type)
                        .param("page", String.valueOf(pageable.getPageNumber()))
                        .param("size", String.valueOf(pageable.getPageSize())))
                .andDo(print())
                .andExpect(status().isOk())
    ...
    void getProductById_ReturnsProductDtoWithStatus200() throws Exception {
        ...
        when(getProduct.getById(id)).thenReturn(product);
